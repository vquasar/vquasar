...
- **M12b — AuthN + RBAC (next).** Multi-user with built-in general roles
  ...
  IdP, but code to standard OIDC): the API becomes an OAuth2 resource server
  validating JWTs; the UI does OIDC Authorization Code + PKCE. **Authorization —
  roles → permissions over our resources — lives in `ch-control`** (map OIDC
  identity/groups to our roles). A **mandatory first-admin bootstrap is set at
  install time** (solves air-gap and the first-admin chicken-and-egg).
